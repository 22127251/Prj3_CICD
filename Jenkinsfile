pipeline {
    ...
    stages {
        stage('Clone github code') {
            steps {
                git branch: 'main', credentialsId: '22127251_github', url: 'https://github.com/22127251/Prj3_CICD.git'
            }
        }
        stage('Build image and upload to dockerhub') {
            steps {
                withDockerRegistry(credentialsId: '22127251_dockerhub', url: 'https://index.docker.io/v1/') {
                    bat 'docker build -t 22127251/prj3_cicd .'
                    bat 'docker push 22127251/prj3_cicd'
                }
            }
        }
        ...
        stage('Pull image from dockerhub and run in container') {
            steps {
                bat 'docker pull 22127251/prj3_cicd'
                bat 'docker run -d -p 3000:80 22127251/prj3_cicd'
            }
        }        
    }
}